...
export default function AssistantChat({
  ...
  const [open, setOpen] = useState(true); // panneau déplié par défaut
  ...
  return (
    <div className="card p-6">
      ...
      >
        <span className="inline-flex w-9 h-9 items-center justify-center rounded-xl bg-brand-950 text-white shrink-0">
      ...
      {open && (
        <>
          <div
            ref={scrollRef}
            className="max-h-72 overflow-y-auto space-y-3 mb-4 mt-4 pr-1"
          >
            {messages.length === 0 && (
              <div className="text-sm text-slate-500">
                <p className="mb-3">
                  Bonjour{firstName ? ` ${firstName}` : ""}, comment puis-je
                  vous aider ?
                </p>
