...
export default function TransferPage() {
  ...
  const handleCreateBeneficiary = async () => {
    ...
    try {
      await createBeneficiary({ clientId: id, name: newBenefName, email: newBenefEmail });
      setNewBenefName("");
      setNewBenefEmail("");
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
        ...
        {mode === "interne" ? (
          <>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Compte destination</label>
              <select
                value={toAccountId}
